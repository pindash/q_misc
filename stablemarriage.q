...
/proper vectorization, we can do better by skipping any impossible matches
f3:{[data;t]
 ms:exec last s by w from t;
 update seen:(seen+1)|min rm by m from data where not m in (0!t)`m,r<ms[w];
 p:select n:m imin r, min r by w from data where rm=seen;
 t upsert select w,m:n, s:r from (p lj t) where r<s}
setup 100;
\t f3[update seen:-1 from `rd]/[{not all key[dm] in (0!x)`m};t]
